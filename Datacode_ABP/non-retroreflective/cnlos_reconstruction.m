...
%         for time-of-flight delays
    ...
    isbackproj = 1; % Toggle backprojection vs LCT (LCT/phasor fields only)
    if alg == 0 
       isbackproj = 1; 
       filter_option = 1;
    elseif alg == 1 || alg == 3 || alg == 4 || alg == 5 
       isbackproj = 1; 
       filter_option = 0;
    end
    snr = 0.1; % SNR value (LCT only)
    ...
    elseif (alg == 3) % phasor fields
        ...
        sampling_coeff = 6; % scale the size of the virtual wavelength (usually 2, optionally 3 for noisy scenes)
